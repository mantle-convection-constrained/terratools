...
if __name__ == "__main__":
    ...
    pressure_bounds = [0., 30.e9, 150.e9]
    temperature_bounds = [200., 2200., 4500.]
    ...
    make_build_files(project_name, molar_composition,
    ...
    if perplex_path != '':
        ...
        im0 = ax[0].contourf(out[:, :, 0]/1.e9, out[:, :, 1],
                             out[:, :, 2], 100, cmap='rainbow')
        im1 = ax[1].contourf(out[:, :, 0]/1.e9, out[:, :, 1],
                             out[:, :, 3], 100, cmap='rainbow')
        im2 = ax[2].contourf(out[:, :, 0]/1.e9, out[:, :, 1],
                             out[:, :, 4], 100, cmap='rainbow')
        ...
        p0 = Q7g.anelastic_properties(elastic_Vp=Vp,
                                      elastic_Vs=Vs,
                                      pressure=P,
                                      temperature=T,
                                      frequency=frequency)
        ...
        Vphi = np.sqrt(Vp*Vp - 4./3.*Vs*Vs)
        out_data = np.array([P, T, Vp, Vs,
                            p0.V_P, p0.V_S, Vphi, rho,
                            p0.Q_S, p0.T_solidus]).T

        header = (f'P(Pa) T(K) Vp(elastic, km/s) '
        ...
        np.savetxt(f'{project_name}_anelastic_properties.dat',
                   out_data, fmt='%.5f',
                   header=header)
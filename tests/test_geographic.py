...
def assert_array_equals_scalar_vec3(testcase, func, args1, args2, args3, atol=1e-7):
    ...
    for arg1, vout1, arg2, vout2, arg3, vout3 in zip(
        ...
        assert_tuples_close(testcase, (vout1, vout2, vout3), (out1, out2, out3),
            atol=1e-7)
...
class TestGeog2Cart(unittest.TestCase):
    ...
    def test_geog2cart_radians(self):
        assert_tuples_close(self, geog2cart(0, 0, 1, radians=True),
            (1.0, 0.0, 0.0))
        assert_tuples_close(self, geog2cart(np.pi/2, 0, 10, radians=True),
            (0.0, 10.0, 0.0))
        assert_tuples_close(self, geog2cart(0, np.pi/2, 100, radians=True),
            (0.0, 0.0, 100.0))
        assert_tuples_close(self, geog2cart(0, 0, 1, radians=True),
            (1.0, 0.0, 0.0))
        assert_tuples_close(self, geog2cart(-np.pi/2, 0, 10, radians=True),
...
class TestCart2Geog(unittest.TestCase):
    ...
    def test_cart2geog_radians(self):
        assert_tuples_close(self, cart2geog(1, 0, 0, radians=True),
            (0, 0, 1))
        assert_tuples_close(self, cart2geog(0, 10, 0, radians=True),
            (np.pi/2, 0, 10))
        assert_tuples_close(self, cart2geog(0, 0, 100, radians=True),
            (0, np.pi/2, 100))
        assert_tuples_close(self, cart2geog(-1, 0, 0, radians=True),
            (np.pi, 0, 1))
        assert_tuples_close(self, cart2geog(0, -10, 0, radians=True),
...
class TestAngularDistance(unittest.TestCase):
    def test_angular_distance_degrees(self):
        self.assertAlmostEqual(angular_distance(0, 0, 0, 10), np.radians(10),
            delta=1e-7)
        self.assertAlmostEqual(angular_distance(0, 10, 0, 0), np.radians(10),
            delta=1e-7)
        self.assertAlmostEqual(angular_distance(0, 90, 0, -90), np.pi,
            delta=1e-7)

    def test_angular_distance_radians(self):
        self.assertAlmostEqual(angular_distance(0, 0, 0, 0.1, radians=True),
            0.1, delta=1e-7)
        self.assertAlmostEqual(angular_distance(0, 0.2, 0, 0, radians=True),
            0.2, delta=1e-7)
        self.assertAlmostEqual(angular_distance(0, np.pi/2, 0, -np.pi/2, radians=True),
            np.pi, delta=1e-7)
    ...
    def test_angular_distance_arrays(self):
        ...
        assert_array_equals_scalar_nargs(self, angular_distance, (lon1, lat1, lon2, lat2))
...
class TestAzimuth(unittest.TestCase):
    ...
    def test_azimuth_radians(self):
        self.assertAlmostEqual(azimuth(0, 0, 0, 0.1, radians=True), 0,
            delta=1e-7)
        self.assertAlmostEqual(azimuth(0, 0, 0.1, 0, radians=True), np.pi/2,
            delta=1e-7)
        self.assertAlmostEqual(azimuth(0, 0, 0, -0.1, radians=True), np.pi,
            delta=1e-7)
        self.assertAlmostEqual(azimuth(0, 0, -0.1, 0, radians=True), -np.pi/2,
            delta=1e-7)

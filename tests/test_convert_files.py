...
def make_old_file(filename):
    ...
    for layer in range(nlayers):
        ...
        for field in fields:
            fields[field]["vals"] = np.random.rand(nlayers,nps).astype(terra_model.VALUE_TYPE)
...
class TestConvertFiles(unittest.TestCase):
    def test_convert_files(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(newfile.dimensions["nps"].size,oldfile.dimensions["nps"].size)
            self.assertEqual(newfile.dimensions["depths"].size,oldfile.dimensions["Depths"].size)
            self.assertTrue(np.all(newfile["temperature"][:,:]==oldfile["Temperature"][:,:]))
            self.assertTrue(np.all(newfile["velocity_x"][:,:]==oldfile["Velocity_x"][:,:]))
            self.assertTrue(np.all(newfile["velocity_y"][:,:]==oldfile["Velocity_y"][:,:]))
            self.assertTrue(np.all(newfile["velocity_z"][:,:]==oldfile["Velocity_z"][:,:]))
            self.assertTrue(np.all(newfile["composition_fractions"][1,:,:]==oldfile["LherzFrac"][:,:]))
            self.assertTrue(np.all(newfile["composition_fractions"][0,:,:]==
